...
st.markdown("""
    ...
    .btn {
        display: block; width: 100%; padding: 7px 0; margin-top: 5px;
        border-radius: 6px; font-size: 12px; font-weight: bold;
        text-decoration: none !important; color: white !important;
        text-align: center; height: 32px; line-height: 18px;
    }
...
def get_photo(url):
    ...
    # Извлекаем ID файла из любой ссылки Google Drive
    match = re.search(r'[-\w]{25,}', str(url))
    if match:
        file_id = match.group()
        # НОВЫЙ МЕТОД: Сервер иконок Google (самый стабильный)
        return f"https://lh3.googleusercontent.com/d/{file_id}"
    return placeholder

@st.cache_data(ttl=1)
def load_data():
    # Твоя ссылка на CSV из Google Sheets
    ...
    if 'ID отдела' in df.columns:
        # Приводим ID отдела к числу (убираем 005 -> 5)
